...
class HMM:
    ...
    def calcgammas(self, alphahats, betahats, cs, obs, uselog=False):
            gammas = []
            for i in range(obs.shape[0]): # for each r
                temp = []
                for t in range(obs[i].shape[0]): #for each t
                    if uselog:
    ...
    def calcxi(self, alphahats, betahats, cs, obs, uselog=False):
        ...
        for i in range(obs.shape[0]): 
            ...
            for t in range(obs[i].shape[0]-1): #time
                for j in range(self.A.shape[0]):# state
                    for k in range(self.A.shape[0]): # state
                        if uselog:
    ...
    def baum_welch(self, obs, niter, uselog=False, prin=0, scaled = True):     
        ...
        for it in range(niter):
            ...
            print("shape of alphahat: " + str(alphahats[0].shape) + " and shape of betahat: " + str(betahats[0].shape) + " and shape of c: " + str(cs[0].shape) )
            print("length of alphahats list: " + str(len(alphahats)) + " and length of betahats list: " + str(len(betahats)) + " and length of cs list: " + str(len(cs))  ) 
            gammas = self.calcgammas(alphahats, betahats, cs, obs, uselog) #alpha*beta*c
            print("shape of gamma: " + str(gammas.shape) )
            newpi = self.calcinit(gammas, uselog) #average of gammas[:,0]
            print("shape of newpi: " + str(newpi.shape))
            xibar = self.calcxi(alphahats, betahats, cs, obs, uselog) #page 132
            print("shape of xibar: " + str(xibar.shape))
            if uselog: 
                xibar = np.exp(xibar)
