...
def featurevisual(data_features, data_labels, data_sampchar, chars=[], plt_num=[], model=False, save=False):
    if len(chars) == 0:
    ...
    for char in chars:
        ...
        if model:
            ...
            f.suptitle('Training characters visualized with features '+data_labels[char], fontsize=10)
        ...
        if save:
            plt.savefig('fig/'+str(data_labels[char])+str(plt_num), bbox_inches='tight', dpi=300)
...
def main():
    ...
    # train_data, test_data, data_labels = dataprep(db_name)

    featurevisual(data_features, data_labels, data_sampchar, chars=[5], plt_num=[3])
