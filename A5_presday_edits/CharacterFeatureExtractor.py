...
def norm_dist_slope(coordinates, thr, legacy=False):
    ...
    # Finding the distance values between strokes ending and starting far from each other and normalizing them to make
    # them similar for different scaled similar symbols
    rel_dist = np.diff(coordinates)
    abs_dist = np.sqrt(    np.square(rel_dist[0,:]) + np.square(rel_dist[1,:])    )
    # if absolute distance between two points is higher than 2 times threshold ( which means there is certainly a jump
    # between strokes) then normalize that distance value
    ...
    # when we have a jump in distance value, we keep that in the same level for remaining distance values coming after
    # that and we apply it for each jumps
...
def featureExtractor(symbol, thr, input_is_dc=True, legacy=True, test=False):
    ...
    if input_is_dc:
        ...
        if test:  # #################################### TEST
            feature_symbol = feature_symbol*100
