...
def hmm_gen(data_features, num_states, useprint=True, longest_sample=False, test=False):
    ...
    featdim = data_features[1][1].shape  # featdim[1] is equal to number of features
    if featdim[1] > featdim[0]:
    ...
    for k in range(K):
        ...
        means = np.zeros([nStates, featdim[1]])
        if not test:  # #################################### TEST
            for i in range(nStates):
